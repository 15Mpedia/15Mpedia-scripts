...
class UploadRobot:
    def __init__(self, url, urlEncoding=None, description=u'',
                 ...
                 ignoreWarning=False, targetSite=None, uploadByUrl=False, ignoreDupes=False):
        """
    ...
    def upload_image(self, debug=False, sessionKey=0):
        ...
        if 'error' in data: # error occured
            ...
            if data['result'] == u'Warning': #upload success but return warning.
                if self.ignoreDupes and len(data['warnings']) == 1 and data['warnings'].keys()[0] == 'duplicate':
                    pywikibot.output("\tIgnoring dupe, uploading")
...
def main(*args):
    ...
    bot = UploadRobot(url, description=description, useFilename=useFilename,
                      ...
                      verifyDescription=verifyDescription, ignoreDupes=ignoreDupes)
    bot.run()
